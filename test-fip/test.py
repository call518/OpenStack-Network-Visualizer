...
#		"pub-compute-004",
	...
	for hostname in hostnames:
		...
		## Compute 호스트일 경우만(snat-xxx 네임스페이스 존재 유무로 판단),  FIP연결성 메타 정보 수집
		if not isNetworkHost:
	...
	## namespace를 key로 해서, VP:rfp-* 인터페이스 이름 조회 가능한 dict 생성
	dvr_fip_ns_rfp_pair = {}
	for item in result_dvr_fip:
		if_name = item['if_name']
		if if_name.startswith("VP:rfp-"):
			if_namespace = item['if_namespace']
			dvr_fip_ns_rfp_pair[if_namespace] = if_name
	...
	for item in result_dvr_fip:
		...
		if if_name.startswith("I:qr-"):
			if_namespace = item['if_namespace']
			if_rfp = dvr_fip_ns_rfp_pair[if_namespace]
	...
	for item in result_dvr_fip:
		...
		if if_name.startswith("VP:fpr-"):
			if_id = item['if_id']
			#print if_id, if_name
			dvr_fip_id_fpr_pair[if_id] = if_name
	#print dvr_fip_id_fpr_pair
	...
	for item in result_dvr_fip:
		...
		if if_name.startswith("VP:rfp-"):
			...
			#print if_pair_id
			if_pair_name = dvr_fip_id_fpr_pair[if_pair_id]
